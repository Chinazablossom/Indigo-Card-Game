...
class `Indigo Card Game` {
    ...
    private fun computerPrompt() {
        println(computersCard.joinToString(" "))
        val randomComputerCard = chooseComputerCard()
    ...
    private fun chooseComputerCard(): Card {
        if (computersCard.size == 1) return computersCard.first()

        val topCard = cardsOnTable.lastOrNull()
        val candidateCards = computersCard.filter { it.suit == topCard?.suit || it.rank == topCard?.rank }

        return when {
            candidateCards.size == 1 -> candidateCards.first()
            candidateCards.size > 1 -> {
                val sameSuitCandidates = candidateCards.filter { it.suit == topCard?.suit }
                if (sameSuitCandidates.size > 1) return sameSuitCandidates.random()

                val sameRankCandidates = candidateCards.filter { it.rank == topCard?.rank }
                if (sameRankCandidates.size > 1) return sameRankCandidates.random()

                candidateCards.random()
            }
            cardsOnTable.isEmpty() -> {
                val sameSuitCards = computersCard.groupBy { it.suit }.values.firstOrNull { it.size > 1 }
                if (sameSuitCards != null) return sameSuitCards.random()

                val sameRankCards = computersCard.groupBy { it.rank }.values.firstOrNull { it.size > 1 }
                if (sameRankCards != null) return sameRankCards.random()

                computersCard.random()
            }
            else -> {
                val sameSuitCards = computersCard.groupBy { it.suit }.values.firstOrNull { it.size > 1 }
                if (sameSuitCards != null) return sameSuitCards.random()
    ...
    private fun displayScore() {
        println("Score: Player $playerScore - Computer $computerScore\n" +
                "Cards: Player ${playerCardsWon.size} - Computer ${computerCardsWon.size}")
    }
    ...
    private fun MutableSet<Card>.resetCard() {
        ...
        loop@ for (i in cardsDeck.shuffled()) {
            ...
            this.add(i)
            newCards.add(i)
            if (counter == numberOfCard) break@loop
        }
